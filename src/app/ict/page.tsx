...
const sections = [
  { key: 'infrastructure', icon: Server, gradient: 'from-ebmc-turquoise to-cyan-500' },
  { key: 'development', icon: Code, gradient: 'from-violet-500 to-purple-500' },
  { key: 'data', icon: Brain, gradient: 'from-emerald-500 to-teal-500' },
  { key: 'devops', icon: Settings, gradient: 'from-orange-500 to-amber-500' }
]
      ...
      <main className="min-h-screen text-slate-800 dark:text-slate-100 overflow-hidden">
        ...
        <section className="relative pt-32 pb-20 overflow-hidden">
          <div className="relative z-10 max-w-7xl mx-auto px-4 text-center">
            ...
            >
              <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-violet-500/10 mb-8">
                <Cloud className="w-4 h-4 text-violet-500" />
                <span className="text-sm font-medium text-violet-500">{t('ict.badge')}</span>
              </span>

              <h1 className="text-4xl sm:text-5xl md:text-7xl font-bold mb-6 text-slate-900 dark:text-white">
                {t('ict.title')}
                <br />
                <TextGradient animate={false}>{t('ict.titleHighlight')}</TextGradient>
              </h1>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="grid md:grid-cols-2 gap-8">
              {sections.map((section, index) => {
                const items = t.raw(`ict.${section.key}.items`) as string[]
                return (
                  <motion.div
                    key={section.key}
                    initial={{ opacity: 0, y: 30 }}
        ...
        <TechSection className="py-20 px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            >
              <div className="w-16 h-16 mx-auto mb-6 rounded-2xl bg-gradient-to-r from-violet-500 to-purple-500 flex items-center justify-center shadow-lg">
